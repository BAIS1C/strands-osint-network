...
export async function fullBriefing() {
  console.error('[S.O.N] Starting intelligence sweep — 29 sources...');
  const start = Date.now();
  ...
  console.error(`[S.O.N] Sweep complete in ${totalMs}ms — ${output.son.sourcesOk}/${sources.length} sources returned data`);
  return output;
}
