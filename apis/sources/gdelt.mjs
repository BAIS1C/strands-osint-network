...
export async function briefing() {
  ...
  try {
    const geo = await geoEvents(
      'conflict OR military OR protest OR crisis OR missile OR strike OR explosion OR sanctions',
      { maxPoints: 250, timespan: '24h' }
    );
    geoPoints = (geo?.features || [])
      .filter(f => f.geometry?.coordinates)
      .map(f => {
        // Strip HTML out of the popup name so it renders clean
        const rawName = f.properties?.name || f.properties?.html || '';
